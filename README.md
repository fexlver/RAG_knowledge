# 基于 Qwen 与 Milvus 的年报 Agentic RAG 工作台

面向上市公司年报资料分散、人工翻阅与数据核验效率低的问题，本项目提供文档入库、混合检索、可观测问答和原文追溯能力。系统不会把“检索到内容”直接等同于“结论正确”：证据相关性不足时会主动拒答，引用可直接打开原文件并定位到对应页或文本段落。
...
年报数据以公告披露为准，自动抽取的数字与状态只能用于检索辅助，不能替代正式披露文件的核验。生产使用还应接入权威数据源、访问控制、操作审计、敏感文档隔离、OCR 和人工复核流程。
